...
--SELECT COUNT(*)
FROM dbo.account a
INNER JOIN prodcopy.account b ON a.SSB_CRMSYSTEM_ACCT_ID = b.SSB_CRMSYSTEM_ACCT_ID__C
...
--SELECT COUNT(*)
FROM dbo.account a
INNER JOIN Kings.dbo.[vwDimCustomer_ModAcctId] b ON a.SSB_CRMSYSTEM_ACCT_ID = b.ssb_crmsystem_acct_id
LEFT JOIN (SELECT crm_id FROM dbo.account WHERE crm_id <> [SSB_CRMSYSTEM_ACCT_ID]) c ON b.ssid = c.[crm_id]
WHERE b.SourceSystem = 'Kings PC_SFDC Account' AND ISNULL(a.crm_id, '') != b.ssid
AND c.[crm_id] IS NULL 
AND b.isdeleted = 0
