...
--exclude Archtics IDs that Joe says is inactive
WHERE        (1 = 1) 
...
--Added this to prevent other accounts from being added in
AND NOT (SourceSystem ='TM' AND CustomerType <> 'Primary')
...
,ISNULL(LEFT(STUFF((    SELECT ', ' + SSID  AS [text()]
FROM (SELECT SSID, [GUID],MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'Eloqua Kings'group by SSID,[GUID]) Eloqua
WHERE Eloqua.[GUID] = z.[GUID] 
ORDER BY Primary_Flag desc, ', ' + SSID
FOR XML PATH('')), 1, 1, ''),8000),'') AS ConcatIDs3
,ISNULL(LEFT(STUFF((    SELECT ', ' + SSID  AS [text()]
FROM (SELECT SSID, [GUID],MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'LA_Cust'group by SSID,[GUID]) Eloqua
WHERE Eloqua.[GUID] = z.[GUID] 
ORDER BY Primary_Flag desc, ', ' + SSID
