...
, COALESCE(CASE WHEN wrlist.isbusinessAccount = 1 THEN NULLIF(CompanyName,'') ELSE NULL END, CASE WHEN dc.SourceSystem = 'GP' THEN NULL ELSE NULLIF(FullName,'') END, ISNULL(FirstName,'') + ' ' + ISNULL(LastName,'')
	+ CASE WHEN ISNULL(Suffix,'') <> '' THEN ' ' + Suffix Else '' END) FullName
, FirstName
, LastName
, CASE WHEN AddressPrimaryStreet = CAST(AccountID as varchar) THEN '' ELSE AddressPrimaryStreet + ' ' + AddressPrimarySuite END AS BillingStreet
, CASE WHEN AddressPrimaryCity = CAST(AccountID as varchar) THEN '' ELSE  AddressPrimaryCity END as BillingCity
, CASE WHEN AddressPrimaryState = CAST(AccountID as varchar) THEN '' ELSE  AddressPrimaryState END as BillingState
, CASE WHEN AddressPrimaryZip = CAST(AccountID as varchar) THEN '' ELSE  AddressPrimaryZip END as BillingPostalCode
, CASE WHEN AddressPrimaryCountry = CAST(AccountID as varchar) THEN '' ELSE  AddressPrimaryCountry END as BillingCountry
